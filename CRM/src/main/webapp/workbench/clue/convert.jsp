...
<script type="text/javascript">
	$(function(){
		...
		/*为创建交易页面的“放大镜”图标绑定时间，打开搜索市场活动模态窗口*/
		$("#openSearchModal").click(function () {
			$("#aname").keydown(function (event) {
				if(event.keyCode==13){
					...
					//展现完列表后，记得将模态窗口默认的回车行为禁用掉（因为模态窗口为回车键默认设置了刷新操作）
					return false
				}
			})
...
<body>
	...
	<div id="title" class="page-header" style="position: relative; left: 20px;">
		...
		<%--以上利用el表达式的隐含对象来获取c对象的值，
		el表达式中，只有xxxScope系列的隐含对象可以省略，
		其他一概不可省略（如果省略了，会变成从域对象中取值--%>
	</div>
	...
	<div id="create-transaction2" style="position: relative; left: 40px; top: 20px; width: 80%; background-color: #F7F7F7; display: none;" >
		...
		<form id="tranForm" action="workbench/clue/convert.do" method="post">
		  ...
		  <div class="form-group" style="width: 400px;position: relative; left: 20px;">
		    ...
		    <input type="text" class="form-control time" id="expectedClosingDate" name="expectediDate">
		  </div>
		  <div class="form-group" style="width: 400px;position: relative; left: 20px;">
